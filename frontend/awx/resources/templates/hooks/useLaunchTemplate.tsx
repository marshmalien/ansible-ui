  ...
  return async (template: Template) => {
    ...
    try {
      ...
      if (canLaunchWithoutPrompt(launchConfig)) {
        ...
        launchRoute = getJobOutputUrl(launchJob as UnifiedJob);
      } else {
        if (template.type === 'job_template') {
          launchRoute = RouteObj.JobTemplatePrompt.replace(':id', template.id.toString());
        } else if (template.type === 'workflow_job_template') {
          launchRoute = RouteObj.WorkflowJobTemplatePrompt.replace(':id', template.id.toString());
        }
      }
